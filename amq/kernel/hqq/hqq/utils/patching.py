...
try:
    ...
    from ..backends.autogptq import patch_hqq_to_gptq_load
except Exception:
    patch_hqq_to_gptq = None

try:
    from ..backends.ft import patch_hqq_to_ft
    from ..backends.ft import patch_hqq_to_ft_load
except Exception:
    patch_hqq_to_ft = None
...
def prepare_for_inference(
    model, allow_merge=False, backend="default", verbose=False, load_path = None
):
    ...
    if backend == "ft":
        if patch_hqq_to_ft is None:
            raise RunTimeError('FT backend is not available.')
        else:
            if load_path is not None and os.path.exists(load_path) is False:
                patch_linearlayers(model, patch_hqq_to_ft, verbose=verbose)
                print("Saving the model to", load_path)
                torch.save(model.state_dict(), load_path)
            elif load_path is not None and os.path.exists(load_path) is True:
                patch_linearlayers(model, patch_hqq_to_ft_load, verbose=verbose)
                print("Loading the model from", load_path)
                model.load_state_dict(torch.load(load_path, weights_only=True))
            else:
                print("No load_path provided, using the model as is")
